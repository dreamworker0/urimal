  ...
  "src/pdf/formula/pipeline.ts"() {
    ...
    FormulaPipeline = class _FormulaPipeline {
      ...
      static async create(options) {
        ...
        const tokenizerJson = JSON.parse(await readFile2(tokPath, "utf-8"));
...
import { resolve, isAbsolute, extname, dirname as dirname2 } from "path";
...
async function parsePdf(buffer, options) {
  ...
  try {
    const mod = await Promise.resolve().then(() => (init_parser(), parser_exports));
  ...
  async ({ file_path }) => {
    try {
      ...
      switch (effectiveFormat) {
        ...
        case "pdf":
          try {
            const { extractPdfMetadataOnly: extractPdfMetadataOnly2 } = await Promise.resolve().then(() => (init_parser(), parser_exports));
  ...
  async ({ file_path, fields, output_format, output_path }) => {
    try {
      ...
      if (output_format === "hwpx-preserve") {
        ...
        if (output_path) {
          mkdirSync(dirname2(resolve(output_path)), { recursive: true });
