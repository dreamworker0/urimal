  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          loadingTask.destroy();
          reject(new (0, _chunkJ3FHRU5Tcjs.KordocError)("PDF \uB85C\uB529 \uD0C0\uC784\uC544\uC6C3 (30\uCD08 \uCD08\uACFC)"));
        }, PDF_LOAD_TIMEOUT_MS);
      })
    ]);
  } finally {
...
async function parsePdfDocument(buffer, options) {
  ...
  try {
    ...
    if (pageCount === 0) throw new (0, _chunkJ3FHRU5Tcjs.KordocError)("PDF\uC5D0 \uD398\uC774\uC9C0\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4.");
    ...
    for (let i = 1; i <= effectivePageCount; i++) {
      ...
      try {
        ...
        if (totalTextBytes > MAX_TOTAL_TEXT) throw new (0, _chunkJ3FHRU5Tcjs.KordocError)("\uD14D\uC2A4\uD2B8 \uCD94\uCD9C \uD06C\uAE30 \uCD08\uACFC");
        parsedPages++;
        _optionalChain([options, 'optionalAccess', _12 => _12.onProgress, 'optionalCall', _13 => _13(parsedPages, totalTarget)]);
      } catch (pageErr) {
        if (pageErr instanceof _chunkJ3FHRU5Tcjs.KordocError) throw pageErr;
    ...
    if (totalChars / Math.max(parsedPageCount, 1) < 10) {
      if (_optionalChain([options, 'optionalAccess', _14 => _14.ocr])) {
        try {
          const { ocrPages } = await Promise.resolve().then(() => _interopRequireWildcard(require("./provider-TEJSQXYE.cjs")));
      ...
      throw Object.assign(new (0, _chunkJ3FHRU5Tcjs.KordocError)(`\uC774\uBBF8\uC9C0 \uAE30\uBC18 PDF (${pageCount}\uD398\uC774\uC9C0, ${totalChars}\uC790)`), { isImageBased: true });
    }
    ...
    let markdown = cleanPdfText(_chunkJ3FHRU5Tcjs.blocksToMarkdown.call(void 0, blocks));
...
function extractBlocksWithGrids(items, pageNum, grids, horizontals, verticals) {
  ...
  if (remaining.length > 0) {
    ...
    if (remaining.length > 0) {
      ...
      const pageH = _chunkJ3FHRU5Tcjs.safeMax.call(void 0, allY) - _chunkJ3FHRU5Tcjs.safeMin.call(void 0, allY);
...
function extractPageBlocksFallback(items, pageNum) {
  ...
  if (clusterResults.length > 0) {
    ...
    if (columns && columns.length >= 3) {
      ...
      const pageHeight = _chunkJ3FHRU5Tcjs.safeMax.call(void 0, allY) - _chunkJ3FHRU5Tcjs.safeMin.call(void 0, allY);
...
function isProseSpread(items) {
  ...
  const maxGap = _chunkJ3FHRU5Tcjs.safeMax.call(void 0, gaps);
...
function detectColumns(yLines) {
  ...
  if (allItems.length === 0) return null;
  const pageWidth = _chunkJ3FHRU5Tcjs.safeMax.call(void 0, allItems.map((i) => i.x + i.w)) - _chunkJ3FHRU5Tcjs.safeMin.call(void 0, allItems.map((i) => i.x));
...
async function applyFormulaOcr(buffer, blocks, pageFilter, effectivePageCount, warnings, _onProgress) {
  const formulaMod = await Promise.resolve().then(() => _interopRequireWildcard(require("./formula-LJP6OF2E.cjs")));
...
//# sourceMappingURL=parser-GGHNKZ4C.cjs.map